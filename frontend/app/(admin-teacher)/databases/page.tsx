...
export default function DatabasesPage() {
  ...
  const role = user?.role as 'admin' | 'teacher';
  const pageTitle = 'Databases';
  const apiService = role === 'admin' ? AdminService : teacherService;
  ...
  const loadData = async () => {
    ...
    try {
      ...
      showNotification(
        `Erro ao carregar cenários e questões. ${error.message || error}`,
        'error',
      );
    } finally {
      setLoading(false);
  ...
  const loadMetrics = async (scenarioId: number) => {
    ...
    try {
      ...
      showNotification(
        `Erro ao carregar métricas de desempenho. ${error.message || error}`,
        'error',
      );
    } finally {
  ...
  const handleSaveScenario = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (scenarioFormData.id) {
        await apiService.editScenario(scenarioFormData.slug, {
  ...
  const handleDeleteScenario = async (slug: string) => {
    ...
    setIsSubmitting(true);
    try {
      await apiService.deleteScenario(slug);
      showNotification('Database excluído com sucesso.', 'success');
      setSelectedScenario(null);
      await loadData();
    } catch (error: any) {
      showNotification(
        `Erro ao excluir database. ${error.message || error}`,
        'error',
      );
    } finally {
      setIsSubmitting(false);
  ...
  const handleSaveQuestion = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        question_number: questionFormData.question_number,
        is_special: questionFormData.is_special,
      };

      if (questionFormData.id) {
        await apiService.editQuestion(questionFormData.id, payload);
            ...
            <div className="w-full lg:w-2/3 bg-white rounded-lg shadow p-6 flex flex-col">
              <div className="flex justify-between items-start mb-6 pb-4 border-b">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() =>
                      handleDeleteScenario(selectedScenario.slug || '')
                    }
                      ...
                      >
                        ...
                        >
                            ...
                            <div className="flex gap-2 items-center">
                              <span
                                className={`text-xs px-2 py-0.5 rounded font-bold ${
                                  q.question_number <= 10
                                    ? 'bg-green-100 text-green-800'
                                    : q.question_number <= 30
                          ...
                          <p className="text-sm font-medium text-gray-800 mb-3">
                            {q.statement || 'Nenhum enunciado disponível.'}
                          </p>
                          ...
                          >
                            {q.expected_query ||
                              'Nenhuma consulta esperada disponível.'}
                          </div>
                        </div>
