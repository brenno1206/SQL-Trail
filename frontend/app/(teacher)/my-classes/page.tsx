...
export default function TeacherClassesPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [classesData, scenariosData, questionsData] = await Promise.all([
  ...
  const loadClassStudents = async (classId: number) => {
    ...
    try {
      ...
      showNotification(`Erro ao buscar alunos da turma.${error}`, 'error');
    } finally {
  ...
  const loadClassMetrics = async (classId: number) => {
    ...
    try {
      const metrics = await teacherService.getClassQuestionsDetail(classId);
      setClassMetrics(metrics);
    } catch (error: any) {
      console.error(`Erro ao buscar métricas da turma.${error}`, error);
      showNotification(`Erro ao buscar métricas da turma.${error}`, 'error');
    } finally {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await teacherService.editClass(selectedClass.id, payload);
      showNotification('Turma atualizada com sucesso!', 'success');
      setIsEditMode(false);
      await loadData();
      setSelectedClass({ ...selectedClass, ...payload });
    } catch (error: any) {
      showNotification(`Erro ao atualizar turma.${error}`, 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDeleteClass = async () => {
    ...
    setIsSubmitting(true);
    try {
      await teacherService.deleteClass(selectedClass.id);
      showNotification('Turma excluída com sucesso!', 'success');
      setSelectedClass(null);
      await loadData();
    } catch (error: any) {
      showNotification(`Erro ao excluir turma.${error}`, 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleEnrollSingle = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      await teacherService.enrollStudent(selectedClass.id, enrollData);
      ...
      showNotification(`Erro ao matricular aluno.${error}`, 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleBulkEnroll = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedClass || !bulkFile) return;
    setIsSubmitting(true);
    try {
      await teacherService.enrollStudentsBulk(selectedClass.id, bulkFile);
  ...
  const handleRemoveEnrollment = async (
    ...
    if (
      ...
      return;
    try {
      await teacherService.removeEnrollment(selectedClass.id, studentId);
  ...
  const openCreateModal = () => {
    setFormData({ class_name: '', subject: '', year_semester: '' });
      ...
      <div className="min-h-screen flex flex-col bg-gray-50 relative">
        ...
        <main className="grow max-w-5xl w-full mx-auto p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800">Minhas Turmas</h1>
            <button
              ...
              {classes.map((classroom) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600">
                    <span className="font-semibold">Professor:</span> Você
                  </p>
                </div>
              ))}
            </div>
                            ...
                            return (
                              ...
                              >
                                <h3 className="text-md font-black text-gray-700 mb-4 uppercase tracking-wider flex items-center gap-2">
                                  <FaMap className="text-gray-500" /> Banco de
                                  Dados: {scenarioName}
                                </h3>
